...
"""This is the implementation of several different CNN-EMGNets"""
"""Optional model_type parameters include: CNN-LSTM, CNN-BiLSTM, CNN-GRU, CNN-BiGRU and CNN-TCN;
   Optional attention parameters include: 'None', 'SE' and 'SA'.
"""


def CNNEMGNets(model_type='CNN-BiLSTM', attention='SA'):
    ...
    if model_type == 'CNN-BiLSTM':
        ...
        Y = TCN(nb_filters=TCN_filters1, kernel_size=3, dilations=(1, 2, 4, 8), return_sequences=True, name='TCN1')(X)
        Y = Dropout(0.2, name='Drop3')(Y)
        Y = TCN(nb_filters=TCN_filters2, kernel_size=3, dilations=(1, 2, 4, 8, 16), return_sequences=False, name='TCN2')(Y)
    else:
