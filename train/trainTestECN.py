    ...
    def train_repeats(self, folds, epoch, target_subject, train_plot, reduce_lr, early_stop):
        ...
        for i in range(folds):
            ...
            if train_plot:
                ...
                verbose = 1
            if reduce_lr:
                reduceLR = ReduceLROnPlateau(monitor='val_capsnet_loss', factor=np.sqrt(0.1), patience=15,
                                             mode='auto', epsilon=0.0001, cooldown=0, min_lr=10 ** -5)
                callbacks.append(reduceLR)
            if early_stop:
                earlyStop = EarlyStopping(monitor='val_capsnet_loss', patience=30, verbose=1, mode='auto',
                                          min_delta=0.00001)
