    ...
    def train_repeats(self, with_transfer, sub_type, folds, epoch, task, target_subject, train_plot, reduce_lr,
        ...
        for i in range(folds):
            ...
            if train_plot:
                ...
                verbose = 1
            if reduce_lr:
                reduceLR = ReduceLROnPlateau(monitor='val_loss', factor=np.sqrt(0.1), patience=30,
                                             mode='auto', epsilon=0.001, cooldown=0, min_lr=10 ** -5)
                callbacks.append(reduceLR)
            if early_stop:
                earlyStop = EarlyStopping(monitor='val_loss', patience=90, verbose=1, mode='auto',
                                          min_delta=0.0001)
