...
class ModelTrainTest:
    ...
    def compile_model(self, model):
        print('model.compile：')
        losses = {'branch_out1': 'categorical_crossentropy', 'branch_out2': 'categorical_crossentropy',
                  'branch_out3': 'categorical_crossentropy', 'branch_out4': 'categorical_crossentropy'}
        metrics = {'branch_out1': 'accuracy', 'branch_out2': 'accuracy',
                   'branch_out3': 'accuracy', 'branch_out4': 'accuracy'}
        model.compile(optimizer=optimizers.adam_v2.Adam(lr=initial_lr), loss=losses,
                      metrics=metrics)
    ...
    def train_repeats(self, folds, epoch, target_subject, train_plot, reduce_lr, early_stop):
        ...
        for i in range(folds):
            ...
            if train_plot:
                ...
                verbose = 1
            if reduce_lr:
                reduceLR = ReduceLROnPlateau(monitor='val_loss', factor=np.sqrt(0.1), patience=10,
            ...
            # Voting
            results = []
            for k in range(len(testPredictResult)):
                results.append(np.argmax(testPredictResult[k], axis=1))
            # print(np.array(results).shape)
            results = most_common_element_per_row(np.array(results).T)
            testPredictResult = to_categorical(results, num_classes=classes)
            # print(testPredictResult.shape)
...
def most_common_element_per_row(matrix):
    result = []
    for row in matrix:
